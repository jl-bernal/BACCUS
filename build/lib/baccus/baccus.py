...
class BACCUS(object):
    ...
    arxiv: ????? to obtain conservative constraints allowing for systematics
        
        
    Parameters
    -----------
        
    prior_std: log(prior) of the diagonal elements of the covariance matrix
        of the shift hyperparameters (std). list of theano functions
        Each function should take sigma_ii as input
        
    prior_corr: log(prior) of the off-diagonal elements of the covariance matrix
        of the shift hyperparameters (correlations). theano function
    ...
    prior_bounds_std: list of tuples with (kindshift,bound_min,bound_max)
    ...
    kind_shifts: type of shifts for each class of datasets. List of tuples of int values
        example: [(1,2),(2),(1,2),(1)]. Must be in order
    
    extras: extra quantities for the lkl - tuple. 
	example: (par1,par2,par3,..)
	It enters as *args in lkl function
    
    want_rescaling: whether to include or not rescaling hyperparameters. True or False
	...
	prepare_prior: Prepare the prior_min and prior_out of all parameters 
	    and hyperparameters and the prior functions for std and rhos in case is none.
	...
	lnprob: Compute the log_lkl calling to lkl, log_prior and inlcuding the
		...
		priors: log(prior) for any sampled parameter other than uniform.
		    List with [model_priors,std_priors,corr_priors]
		    
		nrescaling, nshifts, nsigmas, nrhos: Number (int) of rescaling (0 if
		    want_rescaling is False), shifts, std and corr parameters, respectively
    ...
    def __init__(self, prior_std=None, prior_corr=None, prior_model = [],prior_bounds_model = [],prior_bounds_shifts = [], prior_bounds_std = [], lkl = None,kind_shifts = [],extras=(),want_rescaling=False):
        ...
        self.prior_bounds_std = prior_bounds_std
        self.lkl = lkl
    ...
    def check_input(self):
        ...
        prior_bounds_std = self.prior_bounds_std
        lkl = self.lkl
    ...
    def prepare_prior(self):
        '''
        Prepare the prior_min and prior_out of all parameters and hyperparameters
        and the prior functions for std and rhos in case is none.
        '''
		...
		#prior_shift
        which_shift = []
        std_min = np.zeros(nsigmas)
        std_max = np.zeros(nsigmas)
        ...
        for istd in range(0,nsigmas):
            std_min[istd] = prior_bounds_std[istd][1]
            std_max[istd] = prior_bounds_std[istd][2]                    
        std_min[std_min == 0] += 1e-8
        
        prior_min[ind:ind+nsigmas] = std_min
        prior_max[ind:ind+nsigmas] = std_max
        ...
        #Check prior distributions for std and rho
        prior_std = self.prior_std
        prior_corr = self.prior_corr
        
        #if None, prior_std -> Lognormal (log(s) = N(b,xi))
        if prior_std == None:
            prior_std = []
            b = -6.
            xi = 200.
    ...
    def run_chain(self, DATA, stepsize=2.4, pos=None, nwalkers=300, steps=5000):
        ...
        priors = [prior_model,prior_std,prior_corr]
    ...
    def lnprob(theta, DATA,prior_bounds,priors,nrescaling,nshifts,nsigmas,
        ...
        for isig in range(0,nsigmas):
            sig = theta[ind+isig]
            lp_std += prior_std[isig](sig)

        #Check that params are inside the distributions
        if not np.isfinite(lp_std) or not np.isfinite(lp_corr):
			return -np.inf
        ...
        log_post = log_lkl +lp_corr + lp_std + lp_shift + lp_rescale + lp_model
